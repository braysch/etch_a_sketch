...
static GUID GUID_DEVCLASS_PORTS = {
  0x4d36e978,
  0xe325,
  0x11ce,
  0xbf,
...
void PlotterDevice::HandleRead(uint32_t bytes) {
  ...
  for (auto i = 0u; i < bytes;) {
    ...
    if (newline != nullptr) {
      // Since we found a newline, copy the section prior to the newline into the response buffer
      ...
      if (!_responseBuffer.empty()) {
        ...
        const auto response_str = std::string(_responseBuffer.begin(), _responseBuffer.end());
...
void PlotterDevice::Open(std::wstring & port) {
    ...
    if (!GetCommTimeouts(_portHandle, & timeouts)) throw std::runtime_error("GetCommTimeouts failed.");
          // Wait indefinitely for a byte, return immediately with any data
          timeouts.ReadIntervalTimeout = MAXDWORD; timeouts.ReadTotalTimeoutMultiplier = MAXDWORD; timeouts.ReadTotalTimeoutConstant = MAXDWORD - 1; timeouts.WriteTotalTimeoutConstant = 0; timeouts.WriteTotalTimeoutMultiplier = 0;
          if (!SetCommTimeouts(_portHandle, & timeouts)) throw std::runtime_error("SetCommTimeouts failed.");
            // Start the worker thread
            ResetEvent(_killEvent); _workerThread = std::thread([this] {
              this -> PerformOverlappedIo();
            });
          }
          ...
          void PlotterDevice::PerformOverlappedIo() {
            ...
            while (true) {
              ...
              if (!read_pending) {
                const auto completed_synchronously = ReadFile(_portHandle, & _readBuffer[0], _readBuffer.size(), nullptr, & read_overlapped);
                const auto error = GetLastError();
                if (!completed_synchronously && error == ERROR_IO_PENDING) {
                  read_pending = true;
                } else if (completed_synchronously) {} else {
                ...
                if (!write_pending && !_writeQueue.empty()) {
                  ...
                  WriteFile(_portHandle, write_data.c_str(), write_data.length(), nullptr, &write_overlapped);
                  const auto error = GetLastError();
              ...
              const auto wait_result = WaitForMultipleObjectsEx(wait_handles.size(), & wait_handles[0], false, INFINITE, true);
          ...
          std::wstring PlotterDevice::FindPort() const {
            ...
            const auto info_set = SetupDiGetClassDevs( & GUID_DEVCLASS_PORTS, nullptr, nullptr, DIGCF_PRESENT);
            ...
            for (auto i = 0 u; SetupDiEnumDeviceInfo(info_set, i, & info_data); i++) {
              ...
              if (wcsstr(property_data, L"STMicroelectronics STLink Virtual COM Port") != nullptr) {
                ...
                com_start++; // Skip "(", though we need to search for (COM to prevent it from finding the other COM in the FriendlyName
            ...
            if (last_error != 0 && last_error != ERROR_NO_MORE_DEVICES) throw std::runtime_error("SetupDiEnumDeviceInfo failed.");

...
{
double distance = (((a.first - b.first)*(a.first - b.first)) + ((a.second - b.second)*(a.second
,→ - b.second)));
return distance;
}
...
std::vector<std::pair<int, int>>::const_iterator current =
,→ (sortedEdgeValues.end()-1); //save the value from the starting point
...
for (iterator = edgeValues.begin(); ((iterator != edgeValues.end()) && (currentShortest >
,→ 1.0)); iterator++)
{
